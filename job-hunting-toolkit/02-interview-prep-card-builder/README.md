...
most recent or most senior-sounding. A data bank built across several employers usually has one
program or project that's the most recent, the most impressive-sounding, or the easiest to recall,
and it's tempting to reach for that same one by default rather than genuinely searching the rest
of the data bank for the best-fit proof point per requirement. Before finalizing the Role-Based
and Behavioral tabs, check the spread across employers and across individual accomplishment
entries, not just the spread across JD requirements. If more than roughly a third of the primary
STAR answers trace back to the same single program, that is a signal to pull the fallback angle
from a different employer or a different accomplishment entirely rather than a different angle on
the same one.

> CUSTOMIZE: If you have a specific program, project, or employer in your own data bank that's
> especially recent or easy to over-reach for, name it here the way this section originally named
> one, so Claude knows to actively watch for over-reliance on it specifically.
